  import React, { Component ,Fragment} from 'react'
  import I18n from '../../../../../I18n/I18n'
  import Tags from "react-native-tags"

  import DateTimePicker from "react-native-modal-datetime-picker";
  import moment from "moment"
  import { connect } from 'react-redux'
  //import scrollableTabView , {defaultTabBar ,scrollableTabView}from "react-native-scrollable-tab-view-forked"
  import AsyncStorage from '@react-native-community/async-storage'

  import { View, StatusBar,ActivityIndicator,Platform ,Dimensions,Text,Picker, Image,ScrollView,Switch ,TouchableOpacity ,ImageBackground ,TouchableWithoutFeedback } from 'react-native'
  import { Colors } from '../../../../../Theme'
  import TextInput from '../../../../../Components/Controls/TextInput'
  import Message from '../../../../../Components/Message'
  import ImagePicker from 'react-native-image-picker'
  const country_api = 'https://forums.influancy.com/api/countries';

  import { Formik } from 'formik'
  let token;
  import Icon from "react-native-vector-icons/FontAwesome"

  import LinearGradient from 'react-native-linear-gradient'

  import Button from '../../../../../Components/Controls/Button'
  import * as Yup from 'yup'

  // var {width}=Dimensions.get("window")
  const IS_IOS = Platform.OS === 'ios'
  let iconType = IS_IOS ? 'ios' : 'md'

  // Styles
  import styles from './styles'
  var  TOKEN_KEY= 'USER_TOKEN'
  var USER_KEY= 'USER'
  var d = new Date();
  var year = d.getFullYear();
  var month = d.getMonth();
  var day = d.getDate();
  var since13 = new Date(year -13, month, day)
  var since100 = new Date(year -100, month, day)
  const options = {
    title: 'تغيير الصورة الشخصية',
    // customButtons: [{ name: 'fb', title: 'Choose Photo from Facebook' }],
    storageOptions: {
      skipBackup: true,
      path: 'images',
    },
  };



  var {width} = Dimensions.get("window");
  class EditProfile extends Component {
    constructor(props){
      super(props)
      this.state={
        avatarSource2: this.props.navigation.state.params.profileInfo.user.avatar
        // avatarSource: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR7ITbO8HeQcxa5yhjeacxErtvsy3ibzYzp90k0ogLA_17c7Jw&s",
       , avatarSource1:null,
        isDateTimePickerVisible:false,
        switchValue:true,
        countryList:[],
        cityList:[],
        country_id: 3,
      city_id: "",
      
        day:"",
        month:"",
        year:"",
        gender:"male",
        chosenDate2:"",
        // true for edit profile
        //false for change password
        tabbarstutus:true,

        activetabbarprofile:true,


        activetabbarpassword:false,




      }
    }



componentDidMount(){
  this.setState({
    chosenDate2:this.props.navigation.state.params.profileInfo.birthdate
  })
  

}


    formDefinition = {
      onSubmit: values=> {
        this.props.signup({
          name: values.name,
          email: values.email,
          first_name: values.firstName,
          last_name: values.lastName,
          country_id:values.country_id[0],
          city_id:values.city_id,
          gender:values.gender,
             day: this.state.day,
             month: this.state.month,
             year:this.state.year,
             image:values.image,
             tags:values.tags,
             oldpassword:values.oldpassword,
             newpassword:values.newpassword,
             conf_newpassword:values.conf_newpassword,

   

        })
      },
      initialValues: {
        name: this.props.navigation.state.params.profileInfo.user.userName,
        email: this.props.navigation.state.params.profileInfo.user.email,
        firstName: this.props.navigation.state.params.profileInfo.user.first_name,
        lastName: this.props.navigation.state.params.profileInfo.user.last_name,
          country_id:this.props.navigation.state.params.profileInfo.country.id,
          city_id:this.props.navigation.state.params.profileInfo.city.id,
         day: moment(this.props.navigation.state.params.profileInfo.birthdate).format('D'),
         month:moment(this.props.navigation.state.params.profileInfo.birthdate).format('MM'),
         year:moment(this.props.navigation.state.params.profileInfo.birthdate).format('YYYY'),
        gender: this.props.navigation.state.params.profileInfo.gender, 
        image:this.props.navigation.state.params.profileInfo.user.avatar,
        tags:'',
       

      },
      validationSchema: Yup.object().shape({
        name: Yup.string().required(I18n.t('validation.required')),
        email: Yup.string().required(I18n.t('validation.required')),
        firstName: Yup.string().required(I18n.t('validation.required')),
        lastName: Yup.string().required(I18n.t('validation.required')),
        gender: Yup.string().required(I18n.t('validation.required')),
        birthdate: Yup.string().required(I18n.t('validation.required')),
        country_id: Yup.string().required(I18n.t('validation.required')),
        city_id: Yup.string().required(I18n.t('validation.required')),
        image: Yup.string().required(I18n.t('validation.required')),
        tags: Yup.string().required(I18n.t('validation.required')),




      })
    }

    showDateTimePicker = () => {
      this.setState({ isDateTimePickerVisible: true });
    };

    hideDateTimePicker = () => {
      this.setState({ isDateTimePickerVisible: false ,


      });
    };

    activeprofile=()=>{

      this.setState(
        { activetabbarprofile:true ,
          activetabbarpassword:false}
      )
    }
    activepassword=()=>{

      this.setState(
        { activetabbarprofile:false ,
          activetabbarpassword:true}
      )
    }
    
  uploadimg() {


    ImagePicker.showImagePicker(options, (response) => {
      //console.log('Response = ', response);

      this.setState({
        avatarSource1: response
      }, function () {
        //console.log(this.state.avatarSource1);
      })


      if (response.didCancel) {
        //console.log('User cancelled image picker');
      } else if (response.error) {
        //console.log('ImagePicker Error: ', response.error);
      } else {
        this.setState({
          // avatarSource1:response ,
          avatarSource2: response.uri
        })

      }
    })
  }

    renderGradient = () => (
      <Fragment>
          <LinearGradient
          start={{ x: 1, y: 0 }}
          end={{ x: 0, y: 1 }}
          colors={['#F5F5F5', '#F5F5F5']}
          style={styles.gradientHeader}
        />
        {this.renderBackButton()}
      </Fragment>
    )
    renderBackButton = () => {
      return (
        <Button
          onPress={() => {
            // //console.log(this.props)
            // this.props.navigation.goBack()
            this.props.navigation.navigate('Profile')
          }}
          iconSettings={{
            iconName: `${iconType}-arrow-back`,
            iconSize: 25,
            iconTintColor: Colors.basic.black,
            iconStyle: styles.backButtonIcon
          }}
          type='transparent'
          buttonColor={Colors.rouge}
          containerStyle={styles.backButton}
        />
      )
    }
  componentWillMount(){
      fetch(country_api)
        .then((response) => response.json())
        .then((responseJson) => {
           this.setState({
          //   isLoading: false,
            countryList:responseJson
         }, function(){
          // //console.log(this.state.countryList)
  
          });
  
        })
        .catch((error) =>{
          console.error(error);
        });
    }
    renderForm = () => {
      
      const { fetching, errors } = this.props
      return (
        <Formik
          {...this.formDefinition}
          render={props => {
            return (
            
              <View style={styles.container}>
                <TextInput
                  onChangeText={props.handleChange('name')}
                  value={props.values.name}
                  placeholder={I18n.t('username')}
                  editable={!fetching}
                  style={styles.firstInput}
                />

                <Message name='name' />

                <TextInput
                  onChangeText={props.handleChange('email')}
                  value={props.values.email}
                  placeholder={I18n.t('email')}
                  editable={!fetching}
                  style={styles.firstInput}
                />

                <Message name='email' />

                <TextInput
                  onChangeText={props.handleChange('firstName')}
                  value={props.values.firstName}
                  placeholder={I18n.t('firstName')}
                  editable={!fetching}
                />

                <Message name='firstName' />

                <TextInput
                  onChangeText={props.handleChange('lastName')}
                  value={props.values.lastName}
                  placeholder={I18n.t('lastName')}
                  editable={!fetching}
                  style={styles.firstInput}
                />

                <Message name='lastName' />

                <View style={styles.statusstyle}>
                  <Text style={styles.stutus}>الجنس</Text>
                  <View style={styles.Switchstyle}>
                  <View style={styles.statusstyle}>
                  {/* <Text style={styles.stutus}>الجنس</Text> */}
                  <View style={styles.Switchstyle}>

                <Switch

            value = {!this.state.switchValue}
            trackColor={{ true: '#e81b23', false: Platform.OS=='android'?'#ffffff':'#ffffff'  }}


    thumbColor={[Platform.OS=='ios'?'#FFFFFF':(this.state.switchValue ?'#fff':'#fff')]}

    style={[this.state.switchValue ?styles.switchEnableBorder:styles.switchDisableBorder]}

          onValueChange = {()=>{

            this.setState({
              switchValue:!this.state.switchValue,
              gender:"female"


            },function () {

            
              props.values.gender=this.state.gender

            }
            )
          }}
        />
          <Text>انثى</Text>
                  <Switch

            value = {this.state.switchValue}
          trackColor={{ true: '#e81b23', false: Platform.OS=='android'?'#ffffff':'#ffffff'  }}


  thumbColor={[Platform.OS=='ios'?'#FFFFFF':(this.state.switchValue ?'#fff':'#fff')]}

  style={[this.state.switchValue ?styles.switchEnableBorder:styles.switchDisableBorder]}
          onValueChange = {()=>{



            this.setState({
              switchValue:!this.state.switchValue,
              gender:"male"

            },function () {

              
              props.values.gender=this.state.gender
            }
            )
          }}
          />
          <Text>ذكر</Text>

                  </View>




                </View>




                  </View>

                </View>

                        
                <View>

{/* {alert(this.state.country_id)} */}
{/* {alert(this.state.country_id)} */}

<Picker
  style={{ width: "100%" }}
  selectedValue={(this.state && this.state.country_id ) || 'a'}
  onValueChange={(value, key) => {
// //console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
// //console.log(this.props.navigation.state.params.profileInfo.country.id  )
// //console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
    if (value) {
       props.values.country_id=key
      //console.log(props.values.country_id)
      this.setState({
         country_id:value,
        value1: key
      }, function () {
        ////console.log(this.state.country_id[0]+ this.state.country_id[1])

        if (this.state.country_id != null && this.state.country_id != '')
          fetch(`https://forums.influancy.com/api/cities?code=${this.state.country_id}`)
            .then((response) => response.json())
            .then((responseJson) => {
              //console.log(responseJson)

              this.setState({
                //   isLoading: false,
                cityList: responseJson
              }, function () {
                //console.log(this.state.cityList)

              });


            })
            .catch((error) => {
              console.error(error);
            });
      });
    }

    //  //console.log(this.state.pickerValue)
  }} 
  itemStyle={{ color: 'white' }}>





  {this.state.countryList.map((data) => {
    return (
      <Picker.Item key={data.id} label={data.name} value={data.code}

      />
    )
  })

  }
</Picker>


{this.state.country_id !== "" ?
  <Picker
    style={{
      width: "100%",
    }}
    selectedValue={(this.state && this.state.city_id) || 'a'}

    onValueChange={(value) => {
      if (value !== 0) {
        props.values.city_id = value
        //console.log(props.values.city_id)

        this.setState({ city_id: value }, function () {


        });
      }
    }
    }
    itemStyle={{ color: 'white' }}>




    {this.state.cityList.map((data) => {
      return (
        <Picker.Item key={data.id} label={data.name} value={data.id} />

      )
    })
    }


  </Picker>
  :
  null
} 
</View>

         
      
  

                <View >
                  <Text>{I18n.t('birthday')} </Text>

                  <View style={styles.birthday}>

<View  style={{flexDirection:"column" ,alignItems:"center",justifyContent:"center",width:"100%"}}>
<View style={{width:"50%" ,justifyContent:"center" }}>

                                <Button style={{width:80,margin:10}}
                                buttonColor={Colors.profilecoloropacity}
                                icon={require('../../../../../Images/Icons/events.png')}

                                type="secondary"
                                 title={I18n.t('birthday')} onPress={


                                    this.showDateTimePicker
                                  



                                  }

                              />
                              </View>
  <Text style={{fontSize:18,color:Colors.basic.black , alignItems:"center", textAlign:"center"}}>{
  

 
     
     
  moment(this.state.chosenDate2).locale('en')._i
  } 
    </Text>
  <DateTimePicker
   minimumDate={
             
    new Date(since100) }
    maximumDate={
     
      new Date(since13) }

          isVisible={this.state.isDateTimePickerVisible}
          onConfirm={(data)=>{



              props.values.start_date=data
              this.setState({
                chosenDate2:moment(data).format('DD MM YYYY')
              })
              moment.locale('en')
              
              this.setState({
                chosenDate1:moment(data).format('MMM Do YYYY HH:mm'),
                day:moment(data).format('D'),
                month:moment(data).format('M'),
                year:moment(data).format('Y'),
                
                
              },function() {
                console.log(
                 
                  

                  moment(this.state.chosenDate2).locale('en')._i
                  

     )
                //console.log(`day ${this.state.day} -month ${this.state.month} -year ${this.state.year}`)
              })



            this.hideDateTimePicker();

          }

        }
          onCancel={this.hideDateTimePicker}

          value={props.values.end_date }
          onChangeText={props.handleChange('end_date')}


          datePickerModeAndroid={"spinner"}

        />
                              </View>


                  </View>





                  <View>
                          <Tags
                            editable={!fetching}
                                initialText=""
                                placeholderTextColor={Colors.gray}

                              style={styles.hashstyle}
                                textInputProps={{
                                  placeholder: I18n.t('hashtag')
                                }}

                                initialTags={[]}
                                onChangeTags={tags =>{

                                  props.values.tags=tags
                                  props.handleChange("tags")







                                }}
                                onTagPress={(index, tagLabel, event, deleted) =>
                                  console.log(index, tagLabel, event, deleted ? "deleted" : "not deleted")
                                }
                                inputContainerStyle={{backgroundColor:"white"}}

                                renderTag={({ tag, index, onPress, deleteTagOnPress, readonly }) => (
                                  <TouchableOpacity key={`${tag}-${index}`} onPress={onPress}>
                                    <Text style={styles.hashTagText}>{tag}</Text>
                                  </TouchableOpacity>
                                )}
                                        />
                                        <View>

                                    <Message name='tags' />


                                        </View>

    <View>
      <View style={{width:120}}>
                <Button
                   buttonColor={Colors.profilecolor}
                 

                   type="secondary"
                 
                  onPress={()=>{
                    // //console.log(this.props.navigation.state.params.profileInfo.id)
                    const apiupdateprofile =`https://forums.influancy.com/api/profiles/${this.props.navigation.state.params.profileInfo.id}`

                    var fdata = new FormData();
                    fdata.append("_method","PUT");
                    fdata.append("name",props.values.name);
                    fdata.append("email",props.values.email)
                    if(props.values.first_name ==null){
                      fdata.append("first_name",this.props.navigation.state.params.profileInfo.user.first_name )

                    }else{
                     
                      fdata.append("first_name",props.values.first_name,)

                    }
                    if(props.values.last_name ==null){
                      fdata.append("last_name",this.props.navigation.state.params.profileInfo.user.last_name )

                    }else{
                     
                      fdata.append("last_name",props.values.last_name,)

                    }
                  
                
                    if(this.state.country_id){
                      fdata.append("country_id",`${this.props.navigation.state.params.profileInfo.country.id}` )

                    }else{
                     
                     

                      fdata.append("country_id",`${this.state.country_id[1]}`)
                    }
                    fdata.append("city_id",`${this.state.city_id}`)
                    // ///////////////////
                    if(this.state.day ==''){
                      moment.locale('en')

                      fdata.append("day", moment(this.props.navigation.state.params.profileInfo.birthdate).format('D'),)
                    }else{
                      fdata.append('day', this.state.day ,)
                    }
                    // /////////////////////////
                    if(this.state.month ==''){
                      moment.locale('en')

                      fdata.append("month", moment(this.props.navigation.state.params.profileInfo.birthdate).format('MM'))
                    }else{
                      fdata.append('month', this.state.month )
                    }
                    // /////////////////////////
                    if(this.state.year ==''){
                      moment.locale('en')

                      fdata.append("year", moment(this.props.navigation.state.params.profileInfo.birthdate).format('YYYY'))
                    }else{
                      fdata.append('year', this.state.year )
                    }
                   
                  
                    
                   
                    fdata.append("gender", props.values.gender)
                   try{

                     fdata.append('image', {
                       uri: this.state.avatarSource1.uri,
                       type: 'image/jpeg',
                       name: this.state.avatarSource1.fileName,
                     })
                   }catch{

                   }
                   //fdata.append("tags",props.values.tags)
                   fdata.append("tags","fff")
                    //console.log( " result"+JSON.stringify( fdata));

                    var object = {
                      method: 'POST',
                      headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'multipart/form-data',
                        "Authorization": `Bearer ${token}`, 
                      },
                     body:fdata
                    }

       fetch(apiupdateprofile, object) 
        .then((response) =>{
          //console.log(response)
          this.setState({
            loading1:true
          })
         
          if(response.ok){
            

            this.props.navigation.navigate('Home');
          }
          response.text()
        
        }
        )
        .then((responseData) => {})
        .catch(function(err) {});
          
                    }
                    }
               
                      
                    
                        
                       
                       
                  // loading={fetching}
                  // disabled={fetching}
                  title={I18n.t('save')}



                  />
                  </View>
                  </View>

                  </View>







                </View>


              </View>
            )
          }}
        />
      )
    }

    renderFormeditpassword = () => {
      //console.log()
      const { fetching, errors } = this.props
      return (
        <Formik
          {...this.formDefinition}
          render={props => {
            return (
              <View style={styles.container}>

                <TextInput
                secureTextEntry
                  onChangeText={props.handleChange('oldpassword')}
                  value={props.values.oldpassword}
                  placeholder={I18n.t('current_password')}
                  editable={!fetching}
                />

                <Message name='oldpassword' />

                <TextInput
                secureTextEntry
                  onChangeText={props.handleChange('newpassword')}
                  value={props.values.newpassword}
                  placeholder={I18n.t('new_password')}
                  editable={!fetching}
                />

                <Message name='newpassword' />

                <TextInput
                secureTextEntry
                  onChangeText={props.handleChange('conf_newpassword')}
                  value={props.values.conf_newpassword}
                  placeholder={I18n.t('confirm_password')}
                  editable={!fetching}
                />

                <Message name='conf_newpassword' />
                <View style={{width:120}}>

                <Button

                  buttonColor={Colors.profilecolor}
                

                  type="secondary"
                  onPress={ ()=>{
                      
            if(props.values.newpassword !==props.values.conf_newpassword){
              alert("غير متطابقتين")
            }else{

              var object = {
                method: 'PATCH',
                headers: {
                  'Accept': 'application/json',
                  'Content-Type': 'application/json',
                   "Authorization": `Bearer ${token}`
                  
                },
                body:JSON.stringify( {
                  // check edit 
                  current_password:props.values.oldpassword,     
                  password:props.values.newpassword,
                  password_confirmation:props.values.conf_newpassword,     
                  
                  
        })
      };
      
      // check edit  
      const api=`https://forums.influancy.com/api/user/password/update`
      fetch(api, object)
      .then((response) =>{
        //console.log(response)
         
         if(response.ok){
           
           alert("تم تغيير كلمه السر ")
          //  will be logout to auth email&password 
          // issue

          AsyncStorage.removeItem(TOKEN_KEY)
          AsyncStorage.removeItem(USER_KEY)
          try{
            AsyncStorage.removeItem("profilecheck")
            AsyncStorage.removeItem("SOCILA_MEDIA")
          
        
          }catch{
        
          }
          this.props.navigation.navigate('Auth')
          }
          
          response.text()
        }
        )
        .then((responseData) => {})
        .catch(function(err) {});
        
        
      }}                  
      
    }
                  loading={fetching}
                  disabled={fetching}
                  title={I18n.t('save')}


                  />
                  </View>
                </View>

            )
          }}
        />
      )
    }



    render () {
            // // //console.log(this.props)moment(this.props.navigation.state.params.profileInfo.birthdate).format('D'),
             //console.log(this.props.navigation.state.params.profileInfo.user.first_name)
      return (

      <View style={styles.root}>
        <ScrollView >
          <View style={{paddingHorizontal:20}}>

          <StatusBar barStyle='dark-content' />
          {this.renderGradient()}
        <View style={styles.containeravator}>



          {/* <Image source={{uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjVXybwwswQWwuHWko_d8yyrqqLOYBqlGWkkKNs-yDqqrb2YuQ'}} */}
         
          <Image source={{
           uri: this.state.avatarSource2}}

        style={styles.ImageStyle} />
        <View style={styles.icon}>
        <TouchableOpacity onPress={
         
            () =>
            this.uploadimg()
          


}>
        <Icon  style={styles.icon_style} name="camera" size={25} color="white"/>
        </TouchableOpacity>
        </View>
        
        </View>
        <View style={styles.formstyle}>
          <View style={styles.tabbarstutusStyle}>
            <TouchableWithoutFeedback  onPress={this.activepassword}>

            <Text    style={[styles.changepasswordstyle,this.state.activetabbarpassword ?styles.activetabbar:styles.unactivetabbar]}>
            تغيير كلمة السر  
            </Text>
            </TouchableWithoutFeedback >
            <TouchableWithoutFeedback  onPress={this.activeprofile}>

            <Text   style={[styles.editprofilestyle,this.state.activetabbarprofile?styles.activetabbar:styles.unactivetabbar]}>
            المعلومات الشخصية
            </Text>
            </TouchableWithoutFeedback >

          </View>
          <View>
    {   !this.props &&( 
        
        
       
        <ActivityIndicator
        size='small'
        color={Colors.jungleGreen}
        style={styles.fetching}
        />
     )
      } 
             {this.state.activetabbarprofile?this.renderForm():this.renderFormeditpassword() }
           
      
          </View>
        
        
        
        </View>
        {/* <ImageBackground  source={{uri: 'https://cdn.zeplin.io/5d137bbe05fe2b049f827d9f/assets/bc7786b1-780f-44d5-b1b7-d43d15be5e2f.png'}}
        style={{position:"absolute",width:width,height:200,bottom:0 ,opacity:.3}}>
      
    </ImageBackground> */}
        </View>
        </ScrollView>

        
        </View>
      )
    }
  }


  const mapStateToProps = (state, ownProps) => {

    token=state.login.token
  
    
    
      return {
      
        fetching: state.signup.fetching,
        errors: state.signup.errors
      }
    }

  const mapDispatchToProps = dispatch => ({
    signup: (
      title,
      options0,
      options1,
      options2
  
      
      
    ) =>
      dispatch(
        SignupActions.FormRequest(title,options0,options1,options2)
      )
  })
  
  
  
  EditProfile.defaultProps = {
    signup: () => {},
  
    fetching: false,
    errors: false
  }
  export default connect(
    mapStateToProps,
    mapDispatchToProps
  )(EditProfile)
  


